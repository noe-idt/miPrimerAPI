  ...
  methods: {
    async obtenerDatos() {
      try {
        ...
        const datosCompra = respuesta.data[0];
        this.folio = datosCompra.serie_folio;
        this.descripcion = datosCompra.descripcion;
      } catch (error) {
    ...
    async eliminarCompra() {
      const compra = this.id;
      try {
        await api.get(`http://localhost:8080/api/compras/eliminar/${compra}`, {
          params: {
            api_key: "7aa8e437-4257-468a-8bd5-aafb9396ab53",
          },
        });

        console.log("Compra Eliminado");
      } catch (error) {
        console.log("No se pudo conectar" + error);
      }
    },
